...
しかしWebサービスとインターネット利用者の増加、システムの複雑化によってプロセス単位の処理では限界があることが表面化してきました。1サーバあたりのプロセス数の上限値にあたって処理が捌けなくなったり、そもそもプロセスを作るコスト（メモリなど）が無視できなくなってきたのです。
こうした問題を避けるため、1プロセスで複数のリクエストを処理できるアーキテクチャが注目されました。

:::tips
例えば32bit環境のlinuxサーバでは、プロセスの管理番号の上限に当たるため32768がシステム上の限界値になります。
また１スレッドあたり数MBのメモリを使うため、8GBのメモリを積んだサーバーでは4000ほどが上限になります。
（これらは理論上の話で、当然上限いっぱいになるまで使おうとするとまともに動作しない可能性が高いです。）
:::

特に注目されたのが、非同期I/Oとevent drivenなアーキテクチャを特徴とするNode.jsです。
Node.jsはJavaやRailsと同様にシングルプロセスで動作しますが、I/O待ち中に他のリクエストを処理するようなイメージで複数の処理を行うことができます。

またこの頃から[AWS Lambda](https://aws.amazon.com/jp/lambda/)など、既存のWebサービスインフラとは全く異なるクラウド独自のアーキテクチャが出てきました。

これまでサーバを用意してマルチプロセスによって並列処理を実現していた時代から、シングルプロセスで並列処理を行い、リクエストに応じてプログラムが実行されるfunction型など、より「小さな」単位でプログラムを実行するように流れが変わって行きます。

## Goとマイクロサービス

2018年ごろからの流れとしては、コンテナ化とマイクロサービスが挙げられます。

それまでは開発環境などで使われることが主だったdockerですが、kubernetesなどコンテナオーケストレーションツールが実用化されたことで、production環境をコンテナで提供することが可能になりました。
コンテナオーケストレーションツールを使ったサービスの運用は従来とは全く異なり、宣言的な構成管理と自動化（障害時の自動復旧や高負荷時の自動拡張など）によって、インフラに縛られないダイナミックな運用が可能になります。
kubernetes周辺にはprometheusによるモニタリングなど、巨大なエコシステムが存在します。

コンテナ化に限った話ではありませんが、運用周りの技術発展に伴ってマイクロサービスというアーキテクチャが採用されることが増えました。
従来のように巨大な1つのアプリケーション（モノリスと呼ばれる）を実装するのではなく、ビジネス領域（例えば課金周りだけ別アプリにするなど）や組織単位で小さなアプリケーションを構築し、それらが連携することで1つのサービスを提供する考え方です。
マイクロサービスアーキテクチャには
...
などの利点があります。一方でシステムが分散することで複雑性が増したり、ネットワーク越しに連携することによるパフォーマンス問題など、様々なデメリットも存在します。

この頃から注目されるようになった言語として、[Go](https://golang.org/)や[rust](https://www.rust-lang.org/ja)が挙げられます。
特にGoはgoroutineと呼ばれる軽量なスレッドの仕組みをもち、Webとの相性がいいことからサーバー側の開発言語として採用されることが非常に多くなりました。

これらの言語に共通するのは、静的な型付け機能を持つコンパイル言語という特徴です。
