...
1. 現在でも広く利用されており、今風のフレームワークとしては Laravel が世界的なデファクト。日本では CakePHP も受託開発を中心に根強く使われている

なお補足として、現在の PHP は mod_php ではなく php-fpm（FastCGI Process Manager）で動かすのが標準。
後述のNode.js や Go の「リバースプロキシ + アプリケーションサーバ」と同様に、Webサーバとアプリケーションを別プロセスに分離する構成になっている。
...
1. 1995年Sun Microsystems社（2010年に Oracle が買収）がJava言語を売り出した
...
6. なお Java EE は 2017年に Oracle から Eclipse Foundation へ移管され、現在は「Jakarta EE」という名前になっている
   - この際に商標の都合でパッケージ名がjavax.* から jakarta.* へ一斉に変更され、多くの既存プロジェクトが移行作業に追われた
...
1. 2024年末の Rails 8 では「No PaaS Required」を掲げ、Solid Queue（ジョブキュー）/ Solid Cache / Solid Cable の3点セットが標準化された。
  - デプロイも Kamal（Docker + SSH で任意の Linux サーバへ無停止デプロイ）が同梱される
  - 現在主流のマイクロサービス・クラウドネイティブとは逆行する思想だが、リリース速度や規模感によっては十分に有効
...
#### JavaScript/TypeScriptランタイム事情

Node.js 一強だった状況は変わり、現在は3つのランタイムが並立している。

- **Node.js** … 最も枯れており、エコシステムとホスティングの選択肢が圧倒的。
  迷ったらこれ。現行 LTS は 24 系。
- **Deno** … Node.js の作者 Ryan Dahl が「Node.js の後悔」を踏まえて作り直したもの。
  デフォルトで全権限を拒否するパーミッションモデル（`--allow-net` などで明示的に許可）が最大の特徴。
  Deno 2 以降は npm 互換性を大幅に強化し、`npm:express` のような指定で既存資産も使える。
- **Bun** … JavaScriptCore ベースの高速ランタイム。ランタイム・パッケージマネージャ・
  バンドラ・テストランナーが1バイナリに入っている。2025年12月に Anthropic が買収した
  （Claude Code が Bun 製の単一バイナリとして配布されているため）。
...
ここまでは「1台のサーバの中でどうリクエストを捌くか」の話だった。
2026年の実務では、その上にもう一段「アプリケーションをどう配置・複製するか」の層がある。

一番多いやり方はDocker イメージとしてアプリケーションを固め、Kubernetes などがそれを複数台のサーバに配置し、負荷に応じて増減させる。

この結果、アプリケーション側は「1プロセスで動く」ことに集中し、並列性はコンテナの数で稼ぐという分業が成立した。
「Webサーバのモジュールとして動かす」「プロセスを何個 fork するか」といったかつての悩みの多くは、この層に移動している。

### その他...

- Rust は「OS向けの言語」というイメージが長くあったが、2026年時点では Web バックエンドでも実用段階に入っている。Axum（Tokio 系）がデファクトで、crates.io 自身も Axum で動いている
- とはいえ所有権・ライフタイムの学習コストが高く、多くの Web サービスではGC のある Go / Ruby / Java / TypeScript のほうが多く使われている
- 現実的には「レイテンシやメモリ効率が事業上クリティカルな一部のサービスだけ Rust」という部分採用が行われている
- 昨今はAIプログラミングが大前提の時代に突入しつつある
  - AIに特化した言語やフレームワークが登場するかもしれない
...
SOAP は XML/RPC を発展させ、W3C で標準化されたプロトコル。
WSDL によるインタフェース定義、WS-Security による署名・暗号化などエンタープライズ要件を満たす仕様群が整備された一方、非常に重厚になった。
金融・公共系のシステム連携では今も現役だが、新規採用はまずない。
「厳密なスキーマ定義」という発想自体は gRPC や OpenAPI に受け継がれている。
...
REST や RPC は「クライアントが聞いたらサーバが答える」形式で、サーバから能動的にデータを送れない。これを解決する手段が2つある。

- WebSocket: 双方向・全二重。チャット、共同編集、対戦ゲームなど
  - 接続を張りっぱなしにするため、サーバ側は接続数分の状態を持つことになり、
  - スケールアウト時に「どのサーバに繋がっているか」の管理が必要になる（Redis Pub/Sub などを使う）。
- SSE (Server-Sent Events): サーバ→クライアントの一方向のみ
  - ただの HTTP レスポンスを閉じずに流し続けるだけなので、実装も運用も圧倒的に簡単。

かつては WebSocket のほうが主流だったが、昨今はLLMのレスポンスをSSEで返すのが流行り。
（ChatGPT や Claude の「文字が少しずつ出てくる」あれがSSE）

なお WebSocket や Kafka のような非同期・イベント駆動のインタフェースを
機械可読な形で記述する仕様として **AsyncAPI** がある（OpenAPI の非同期版のような位置づけ）。
...
ここまでのAPIは「人間が書いたプログラムが呼ぶ」ことを前提にしていた。
2026年現在、そこに 「AIが呼ぶ」 という新しい利用者が加わっている。

LLM に外部の機能を使わせるには、「どんな機能があるか」「引数は何か」をLLMが理解できる形で渡す必要がある。
これを各社バラバラにやっていたところを標準化したのが MCP で、
Anthropic が2024年11月に公開し、その後 OpenAI・Google なども採用して事実上の標準になった。

MCP サーバは「ツール（実行できる操作）」「リソース（読めるデータ）」などを公開し、
AI 側（MCP クライアント）がそれを発見して呼び出す。
通信自体は JSON-RPC が使われる。
