...
PerlCGIから始まり、Webアプリケーション開発の大まかな流れを追ってきた。
ここ数年の流れとしては、2015年頃？に富豪プログラミング＋動的型付けによる素早い開発が流行した流れを受け、小さく高速な処理と静的型付けなどによるバグの少ない開発を目指すのが主流になりつつある（個人の感想です）。

一方でRailsなどのフレームワークを使ったときの開発効率は今でも非常に優秀で、サービスのスタートアップではRailsを採用するケースがまだまだあると思われる。

個人的な所感をざっくりまとめると以下のようになる。

- 手堅く大規模な開発に向いたJava
- 小さめで高速なアプリケーションを作るのに向いたGoとNode.js
- 開発効率に優れたRailsやDjango
- フロントまで含めて軽量で素早い開発が可能なPHP

ただしどの言語を採用するかはサービス全体のアーキテクチャや開発スタイルにもよるため、周辺事情もよく考える必要がある。
