...
- [FastAPIを使ったAPIサーバ作り](#fastapiを使ったapiサーバ作り)
  ...
  - [演習解説](#演習解説)
  - [演習5. FastAPIでWeb APIを作る](#演習5-fastapiでweb-apiを作る)
    - [5.0 エンドポイントの追加](#50-エンドポイントの追加)
...
## 演習5. FastAPIでWeb APIを作る

それではいよいよ本格的にAPIの開発を行っていきます。
FastAPIに限らずAPIを作る以上、ただの静的データを返すだけでは意味がありませ
ん。
APIである以上、アクセスする際に何らかのパラメータを受け、それに応じた応答を返す必要があります。
ここでは代表的な3つの例を挙げて作ってみることにします。

### 5.0 エンドポイントの追加

先ほどの例ではトップページを作りましたが、APIは全て"/"で受け付けるわけではありません。
従って、他のURLパスでも応答を返せるようにエンドポイントを追加します

- 追記する内容
  ```python
  @app.get("/endpoint/{item_id}")
  def read_endpoint():
      return {"mssages": "endpoint"}
  ```
- 追加した情報の再読み込み
  - Ctrl+Cで先ほどのアプリケーションを停止し、以下の通り再起動する
    ```bash
    uvicorn main:app --host 0.0.0.0
    ```
- 追加したエンドポイントへアクセス(別ターミナルを起動しfastapiコンテナにログイン)
  ```bash
   curl --noproxy 127.0.0.1 http://127.0.0.1:8000/endpoint

  {"mssages":"endpoint"}
  ```

<details>
<summary> main.py全容</summary>
...
APIを作る際にパス情報をパラメータとして何らかの処理をしたい場合を考えます。
...
さて、ここまででFastAPIを用いて作るAPIの基本的な部分は学びました。
...
FastAPIはAPI開発用フレームワークという事もありデフォルトでは全てJSONで返す挙動となっています。
...
起動をGunicornに変更できたら次はASGI対応のAPIを作ってみましょう。
実はuvicornでもASGI対応のAPI作成は可能なのですが、前述の通りGunicornを使え、と言うのが
公式の推奨という事もあり、まずはGunicornの活用を先に行っています。

また、ASGIとはWSGIと呼ばれるWebサーバとAPIを接続するための、
...
今回は、FastAPIを使ってAPIを作ってみました。
FastAPIは決して大規模なAPIを作るために作られた物ではありませんが、作り方次第で多くの処理を任せることができます。
