...
つまりthread1が1を実行した後、たまたまコンテキストスイッチが発生しthread2が1を実行する可能性があります。つまりPythonにおける`a += 1`はレースコンディションを起こす可能性があり、「スレッドセーフではない」ことが分かります。
...
- ロックなど排他処理の実装を行う際は自前で実装せず、必ずライブラリを利用しましょう
