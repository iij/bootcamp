...
作成する関数は、渡されたモノの名前をオウム返しする関数です。  
本講義で行えることは限られるので、行為を以下のように定義します。  
* 空白以外の文字列を全てモノの名前とみなす
* 話す行為は、標準出力とする
	* 本章では、`fmt.Println` を用いて、文字列を出力します
* 話せる内容の場合は、`true`, そうでない場合は、`false`を返す  
...
```shell
:# TERMINAL 0
:# WORKPATH /go/src/go_tutorial/4_func/goods/

$ cd /go/src/go_tutorial/4_func/goods/
$ <お好きなエディタ> eaters.go
$ go run eaters.go
```
* `/go/src/go_tutorial/4_func/monkey/eaters.go`
	```go
	package main
...
```shell
:# TERMINAL 0
:# COPY /go/src/go_tutorial/4_func/monkey/eaters.go /go/src/go_tutorial/4_func/likego/eaters.go
:# WORKPATH /go/src/go_tutorial/4_func/likego/

$ cp /go/src/go_tutorial/4_func/monkey/eaters.go /go/src/go_tutorial/4_func/likego/eaters.go
$ cd /go/src/go_tutorial/4_func/likego/
$ <お好きなエディタ> eaters.go
$ go run eaters.go
```
...
なお、Go言語のstderr定数は、`os`パッケージの、`os.Stderr`として存在するため、追加で、`os`パッケージをインポートしてください。  
