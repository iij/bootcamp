...
## 0. この講義について
### 0.1. この講義の目的
Goについての概要の解説とGoでWebアプリケーションを作ることを通して特徴を知り、サービスや手元ツールなどの実装言語選択時の候補に挙げられるようになることを目的としています。

### 0.2. ハンズオンの対象者
開発部署だけでなく、今後は運用等の部署でもプログラミングの能力が求められることが多くなることが予想されます。 \
GoはWebサービスだけでなく、配布が楽という面で部署内で使う小さな便利ツールを作る際にも有効な選択肢となり得ます。 \
そのため対象の部署に制限はありません。 \
...
チュートリアルリポジトリの`hello_world`ディレクトリにもサンプルコードが入っています。
...
### 2.1.2. 実行ファイルの作成
:computer: 好きなエディタで以下のファイル(`go_tutorial/hello_world/main.go`)を作成しましょう。
...
:computer: 実行ファイルには`main`という文字列が3箇所表れます。これらのうち、一言一句`main`という文字列でなければならないところはどこなのか確認しましょう。
...
これらを一つずつ`main`から適当な文字列に変えながら2.1.3.と同様に`go run`を実行し、実行できるかどうか、実行できなかった場合はどのようなエラーが出力されるのかを確認しましょう。
