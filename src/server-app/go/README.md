...
本講義では、最後に、開発いただいたhttpサーバを用い、並行処理を体験してもらおうと思います。  

まず先に、並行処理についてイメージいただきます。  
...
並列プログラミングなコードを自身で作成するためには、領域の管理を考えた数十行のコードを作成する必要があります。  
牛丼屋の例で考えると、座席をどう確保すべきか、どういった要素が必要か。客が離れたら座席をどのように撤去するべきか。1つしかない厨房から牛丼をどのような形で提供するか。辺りです。  

既に特徴で紹介していますが、Go言語では、Goroutine を用いることで、とても低コストに並列プログラミングを行えます。  
:rocket: Goroutineだけではなく、本講義では触れないChannelや、GCによって、低コストに並列プログラミングができます。  

実際に、食べる処理(Eat関数) を、並列プログラミングにして、2つ目のリクエストが、1つ目のリクエストを待たなくても良いように、アップグレードしましょう  

### 7.4.4. :computer: 並行動作するhttpサーバ体験
...
実は、本講義を通して、Go標準パッケージの劣化版を、Go標準パッケージに近づける開発をいただきました。  
