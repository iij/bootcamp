...
この特徴を活かし、Goでは、戻り値の最後に関数処理エラーの有無を識別できる値(error型)をつける形がよく使われます。  
なお、エラーが無い場合は、error型の値を`nil`として渡します。  
...
### 7.4.1 :computer: 1座席なWebアプリケーションサーバ体験
...
### 7.4.4. :computer: 並行動作するWebアプリケーションサーバ体験
