...
配列の`map`メソッドは例えば以下のように使われるメソッドです。
...
このパターンはReactでの開発にとてもよく現れる上に、慣れるまでは
...
ReactではHookをまとめて独自のHookを定義でき、これをカスタムHookと呼びます。
Reactではこれを利用し「状態とそれに対する操作」をまとめてカスタムHookとして定義し、処理を抽出することをよくやります。
...
ローカルでサーバを動かすためのコンテナイメージを用意するつもりですが間に合うかは不明です。
