...
上記1以外は、リレーショナルデータベースではないことから、[NoSQL](https://ja.wikipedia.org/wiki/NoSQL)と呼ばれることもある

# データへのアクセス手段
1. リレーショナルデータベース
   * SQL
     * SQLには、ANSI/ISOで決められた標準SQLと呼ばれるものがある。標準SQLは、決められた年をつけて表記され、現在の最新は「SQL:2016」
