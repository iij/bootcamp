...
前回の事前準備でDockerコマンドのインストールを実施いただきました。

下記コマンドを入力し、コマンドが実行できるかどうか確認してください。
...
proxyを設定している場合は下記のように`--noproxy`オプションを指定する必要があります。
