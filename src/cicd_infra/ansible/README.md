...
- [Ansible による IT自動化](#ansible-による-it自動化)
  ...
  - [3. Ansible playbook の作成](#3-Ansible-playbook-の作成)
  - [4. Ansible によるサーバセットアップ](#4-ansible-によるサーバセットアップ)
  - [5. Ansible 設定ファイルの管理](#5-Ansible-設定ファイルの管理)
...
講義当日までに以下のリポジトリを参照し、Docker コンテナによる演習環境を整えてください。
教材は[こちら](https://github.com/iij/ansible-exercise)になります。
...
この項では、Ansible コンソールコンテナへのログイン・ログアウト方法についても解説します。
演習中は何度もコンテナへログインする場面がありますので、手順を覚えておきましょう。

### システム構成

このハンズオンでは、複数の Docker コンテナを仮想的なVMとして扱い、演習を進めます。
下記は、手順通りに演習環境を構築した際の構成図です。

![alt text](images/env-image.png)

受講者のマシンは Ubuntu 24.04 が稼働する仮想マシンを想定していますが、同等の環境なら何でも構いません。

#### コンテナへのログイン方法

- ターゲットノードとなるコンテナへログインしたい場合は、対象コンテナ名を指定して `docker compose exec` コマンドを利用します。
...
- コンテナ内からログアウトするには、`exit` コマンドを入力または Ctrl + D を押下します。

  ```bash
  exit
  ```

- ログアウトすると、元のホスト（ホストマシンのシェル）に戻ります。
